        ...
        <div className='container mx-auto flex flex-col lg:flex-row'>
          <div className="lg:w-1/2">
            <div className={`productImage flex items-center justify-center lg:justify-start border-2 border-gray-100 rounded-lg ${fade ? 'fade-out' : ''}`}>
              <img className={`w-full max-w-full lg:max-w-[500px] rounded-lg ${fade ? 'fade-out' : ''}`} src={selectedPhoto ? selectedPhoto : product?.images[0]} alt="Product" />
            </div>            

            <div className="sub-images flex flex-wrap mt-4 justify-center">
              <img className="w-1/3 lg:w-auto lg:max-w-[110px] rounded-lg p-1 border-[1px] border-gray-100"
          ...
          <div className="lg:w-1/2 mt-6 lg:mt-0 lg:ml-6">
            <div className="productName mb-2 font-bold text-[40px] mb-2 max-w-[400px] lg:mx-0 lg:my-0" style={{color:"#002D46"}}>{product?.name}</div>

            <div className="description mt-2 mb-4 text-[18px]" style={{color:"#919291"}}>{product?.description}</div>

            <div className="price flex">              
              <div className="present-cost mr-2 font-bold" style={{fontSize:"28px", color:"#002D46"}}>&#8377;{product?.price-(product?.price * (product?.discount/100))}</div>
            ...
            <div className="alltax text-sm mb-2" style={{marginTop: "-0.5rem", color:"#002D46"}}>Inclusive of all taxes</div>
            ...
            <div style={{display:"flex", flexDirection:"row", gap:"16px"}} className="cartAndBuy flex flex-col lg:flex-row mb-3">
              ...
              <button className='button_cart1 buyNow shining-btn flex-grow text-white font-bold py-3 px-3 rounded-lg border' style={{backgroundColor:"#52B6AA"}}>Buy Now</button>
            </div>

            <button className="w-full py-3 rounded-3xl text-white font-semibold mb-2 mt-2 shining-btn border-[15xl] mt-4" style={{background: "conic-gradient(from -24deg at 98.3% 0%, #8fc5be 82.800002deg, #6db4aa 133.89502deg, #fff 359.9063801765442deg)"}}>
              <p className='bulk text-[28px] text-black'>Bulk Order Query</p>
              <p className='corporate text-[14px] text-black'>Corporate Orders</p>
            </button>
          </div>
        </div>  
            ...
            <div className="main-box w-full bg-[#F0F3F4] p-[12px] rounded-[12px]">
              <ul>
                ...
                <li className="flex justify-between mb-2">
                  ...
                  <span className="w-[65%] lg:w-[80%]">{product?.notification}</span>
                </li>
                <li className="flex justify-between mb-2">
